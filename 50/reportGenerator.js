...
export async function exportReportAsPDF() {
    ...
    if (pdfMode === 'brief') {
        // In brief mode, remove the expander buttons and the detail tables
        ...
        // In full mode, make sure the detail tables are visible for rendering
    ...
    const margins = { top: 30, bottom: 20, left: 15, right: 15 };
    const contentWidth = doc.internal.pageSize.getWidth() - margins.left - margins.right-100;

    // Use a temporary div to render the clone off-screen with a fixed width
    ...
    await doc.html(reportClone, {
        ...
        y: margins.top,
        width: contentWidth,
        windowWidth: 650, // Match the temporary div width
    ...
    // Add screenshots only in 'full' mode
if (pdfMode === 'full' || pdfMode === 'brief'  ) {
        ...
        if (selectedLevels.length > 0) {
            ...
            for (const level of selectedLevels) {
                ...
                doc.setFontSize(16);
                doc.text(`${level.replace(/_/g, ' ')} Plan`, margins.left, margins.top - 10);
                
                const scProps = doc.getImageProperties(screenshotData);
                let scHeight = (scProps.height * contentWidth) / scProps.width;
                let scWidth = contentWidth;
                const maxImgHeight = doc.internal.pageSize.getHeight() - margins.top - margins.bottom;
                
                if (scHeight > maxImgHeight) {
                    scWidth = (scWidth * maxImgHeight) / scHeight;
                    scHeight = maxImgHeight;
                }
                
                doc.addImage(screenshotData, 'JPEG', margins.left, margins.top, scWidth, scHeight);
            }
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        addWatermark(doc);
        doc.setFontSize(8);
        doc.text(`Page ${i} of ${pageCount}`, doc.internal.pageSize.getWidth() / 2-15, doc.internal.pageSize.getHeight() - 10, { align: 'center' });
    }
