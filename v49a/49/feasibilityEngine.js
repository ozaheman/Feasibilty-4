        ...
        if (isAptModeManual) {
            ...
            if (Object.values(manualCountsPerFloor).some(c => c > 0)) {
                 typicalFootprints.forEach((footprint) => {
                    // This is an approximation for corridor area. It lays out the total units per floor on each wing.
                    // The corridor area calculation is primarily dependent on polygon shape and average unit depth, so it's a reasonable estimate.
                    const layoutResult = layoutFlatsOnPolygon(footprint, manualCountsPerFloor, includeBalconiesInOffset, calcMode, doubleLoaded);
