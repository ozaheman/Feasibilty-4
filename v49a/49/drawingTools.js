    ...
    switch (state.currentMode) {
        ...
        case 'drawingLinearBuilding':
            if (currentDrawingPolygon) {
                ...
                if ((state.currentMode === 'drawingBuilding' || state.currentMode === 'drawingLinearBuilding') && isLayoutLevel && newPoints.length > 1 && state.currentProgram) {
                    ...
                    // Perimeter calculation for preview
                    let tempPerimeter = 0;
                    if(state.currentMode === 'drawingLinearBuilding') {
                        // For polyline, calculate length of segments
                        for(let i=0; i<newPoints.length-1; i++) {
                            tempPerimeter += Math.hypot(newPoints[i+1].x - newPoints[i].x, newPoints[i+1].y - newPoints[i].y);
                        }
                    } else {
                        tempPerimeter = getPolygonProperties(tempFabricShape).perimeter;
                    }
                    // Scale to meters
                    tempPerimeter *= state.scale.ratio;
                    ...
                    if (avgFrontage > 0) {
                        ...
                        liveLayoutData = layoutFlatsOnPolygon(tempFabricShape, counts, includeBalconies, calcMode, doubleLoaded);
                        
                        // MODIFIED: Display live corridor area on status bar
                        if (liveLayoutData?.corridorArea > 0) {
                            document.getElementById('status-bar').textContent = `Drawing... Est. Corridor Area: ${f(liveLayoutData.corridorArea)} m²`;
                        }
...
export function handleDblClick(o) {
    // MODIFIED: Handle vertex removal on double click
    if (state.currentMode === 'editingFootprint' && o.target && o.target.isFootprint) {
        const activePolygon = o.target;
        const pointer = state.canvas.getPointer(o.e);

        // Find which vertex control was clicked
        for (const key in activePolygon.controls) {
            if (key.startsWith('p')) {
                const control = activePolygon.controls[key];
                const corner = { x: control.x, y: control.y };
                const dist = Math.hypot(pointer.x - corner.x, pointer.y - corner.y);
                const threshold = control.sizeX ? control.sizeX / 2 : 10 / state.canvas.getZoom();

                if (dist < threshold) {
                    const index = parseInt(key.substring(1));
                    if (activePolygon.points.length > 3) { // Must maintain at least 3 points
                        activePolygon.points.splice(index, 1);
                        refreshEditablePolygon(activePolygon); // Refresh controls
                        state.canvas.renderAll();
                        handleObjectModified({ target: activePolygon });
                    }
    ...
    // Handle both closed polygon and linear polyline finishing on double click
    if ((state.currentMode === 'drawingPlot' || state.currentMode === 'drawingBuilding' || state.currentMode === 'drawingLinearBuilding') && polygonPoints.length > 1) {
        ...
        // Remove duplicate last point if it exists (from the double click sequence)
...
// NEW: Functions moved from eventHandlers.js to break circular dependency
export function makeFootprintEditable(polygon) {
    ...
    refreshEditablePolygon(polygon); // Use the new helper
...
export function refreshEditablePolygon(polygon) {
    polygon.controls = fabric.Object.prototype.controls; // Reset to default first
    state.canvas.renderAll(); // Rerender to remove old controls
    polygon.controls = {}; // Clear controls object
    ...
    // Set object coordinates to force control recalculation
    polygon.setCoords();
}
