...
import { allocateCountsByPercent, getPolygonProperties, getOffsetPolygon, isPointInRotatedRect } from './utils.js';
...
function handleBalconyClick(pointer) {
    ...
    for (const flat of state.currentApartmentLayout.placedFlats) {
        if (!flat.hasBalcony || !flat.type.balconyMultiplier || flat.type.balconyMultiplier <= 0) continue;
...
export function handleMouseDown(o) {
    ...
    // NEW: Add vertex to polygon on edge click in edit mode
    if (state.currentMode === 'editingFootprint') {
        const activePolygon = state.canvas.getActiveObject();
        if (activePolygon && activePolygon.isFootprint) {
            const edgeIndex = getClickedPolygonEdge(activePolygon, pointer);
            if (edgeIndex > -1) {
                // Convert pointer to object's local coordinates
                const localPointer = fabric.util.transformPoint(
                    pointer,
                    fabric.util.invertTransform(activePolygon.calcTransformMatrix())
                );
                // Insert the new point into the points array
                activePolygon.points.splice(edgeIndex + 1, 0, localPointer);
                // Refresh the editable controls
                refreshEditablePolygon(activePolygon);
                state.canvas.renderAll();
...
////not implemented/////
// NEW: Helper function to refresh controls after adding/removing a vertex
// refreshEditablePolygon
// makeFootprintEditable
// makeFootprintUneditable
