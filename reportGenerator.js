...
export async function captureLevelScreenshot(levelName, multiplier = 1.0) {
    ...
    // Draw a white background to prevent transparency issues with JPEG
    ctx.fillStyle = 'white';
...
export async function exportReportAsPDF() {
    ...
    const margins = { top: 30, bottom: 20, left: 8, right: 8 };
    const contentWidth = doc.internal.pageSize.getWidth() - margins.left - margins.right-5;
    const contentHeight = doc.internal.pageSize.getHeight() - margins.top - margins.bottom;

    // 1. Generate main report pages using jsPDF's robust HTML renderer
    ...
    if (selectedLevels.length > 0) {
        ...
        for (const level of selectedLevels) {
            ...
            const footerY = doc.internal.pageSize.getHeight() - margins.bottom + 5;
            doc.setFontSize(8);
            doc.text(`${level.replace(/_/g, ' ')} Plan`, margins.left, margins.top );

            const scProps = doc.getImageProperties(screenshotData);
            //let scHeight = (scProps.height * contentWidth) / scProps.width;
            let scHeight = (scProps.height * contentWidth) / scProps.width;
            let scWidth = contentWidth-margins.left-margins.right;
            ...
            doc.addImage(screenshotData, 'JPEG', margins.left, margins.top, scWidth, scHeight-margins.bottom-margins.top);
        }
        document.getElementById('status-bar').textContent = 'Screenshots added.';
    }

    // 3. Add headers, footers, and watermarks to ALL pages
