...
export function generateSummaryReportHTML(data) {
    ...
    const { inputs, areas, aptCalcs, hotelCalcs, schoolCalcs, summary, parking, lifts, staircases, levelBreakdown } = data;
    ...
    if (schoolCalcs) {
        schoolReportHTML = `
        <table class="report-table"><tr class="section-header"><td colspan="3">School Play Area (SQM)</td></tr>
            <tr><th>Description</th><th>Required</th><th>Provided</th></tr>
            <tr><td>Total Play Area</td><td>${f(schoolCalcs.playAreaRequired)}</td><td class="${schoolCalcs.playAreaProvided >= schoolCalcs.playAreaRequired ? 'surplus' : 'deficit'}">${f(schoolCalcs.playAreaProvided)}</td></tr>
            <tr><td>Covered Play Area</td><td>${f(schoolCalcs.coveredPlayAreaRequired)}</td><td class="${schoolCalcs.coveredPlayAreaProvided >= schoolCalcs.coveredPlayAreaRequired ? 'surplus' : 'deficit'}">${f(schoolCalcs.coveredPlayAreaProvided)}</td></tr>
        </table>
