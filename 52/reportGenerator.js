...
// NEW: Check the state of the toggles
    ...
    const includeRenting = document.getElementById('toggle-revenue-renting').checked;
     const reportHTML = isDetailed 
        ? generateDetailedReportHTML(calculatedData, costParams, includeCost, includeBuying, includeRenting) 
...
export function generateDetailedReportHTML(data, costParams, includeCost, includeBuying, includeRenting) {
    if (!data) return '<p>Calculation failed. Please check inputs and drawings.</p>';
    ...
    if (includeCost) {
        ...
        const landCost = plotAreaSqft * costParams['cost-land'];
        ...
        <table class="report-table" style="margin-top: 20px;">
            ...
            <tr><td>Land Cost</td><td>${fInt(plotAreaSqft)} Plot Area sqft @ ${fInt(costParams['cost-land'])}/sqft</td><td>${fInt(landCost)}</td></tr>
            ...
            <tr><td>Electrical Connection</td><td>Fixed</td><td>${fInt(electricalCost)}</td></tr>
    ...
    if (includeBuying) {
        if (state.lastMarketRates || state.offlineMarketRates) {
            const marketRates = state.lastMarketRates || state.offlineMarketRates[Object.keys(state.offlineMarketRates)[0]]; // Fallback logic
            let totalRevenue = 0;
            let revenueRows = '';
// Residential Revenue
                ...
                aptCalcs.aptMixWithCounts.forEach(apt => {
                   const marketData = marketRates[apt.key];
                    if (marketData && apt.totalUnits > 0) {
                        ...
                        revenueRows += `<tr><td>${apt.type}</td><td>${fInt(apt.totalUnits)} units</td><td>${fInt(marketData.buy)} /unit</td><td>${fInt(revenue)}</td></tr>`;
                    }
                });
            }
...
let rentingRevenueHTML = '';
    if (includeRenting) {
        if (state.lastMarketRates || state.offlineMarketRates) {
            const marketRates = state.lastMarketRates || state.offlineMarketRates[Object.keys(state.offlineMarketRates)[0]];
            ...
            if (state.projectType === 'Residential' && aptCalcs) {
                 aptCalcs.aptMixWithCounts.forEach(apt => {
                    ...
                    if (marketData && apt.totalUnits > 0) {
                        ...
                        rentRows += `<tr><td>${apt.type}</td><td>${fInt(apt.totalUnits)} units</td><td>${fInt(marketData.rent)} /yr</td><td>${fInt(rent)}</td></tr>`;
                    }
                });
            }
    ...
    <tr class="section-header">
        ...
        <tr class="total-row">
             ...
             <td><strong>Totals</strong></td><td><strong>${f(totals.sellableGfa)}</strong></td><td><strong>${f(totals.commonGfa)}</strong></td><td><strong>${f(totals.service)}</strong></td><td><strong>${f(totals.parking)}</strong></td><td><strong>${f(totals.balconyTerrace)}</strong></td><td><strong>-</strong></td>
        </tr>
    </table>
